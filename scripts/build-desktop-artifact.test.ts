...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("keeps Experimental product identity across release versions", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "T3 Code (Experimental)");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "T3 Code (Experimental)");
  });
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(mac.dmg, {
        title: "T3 Code (Experimental) 1.2.3 Installer",
  ...
  it.effect("skips the primary native probe for cross-architecture Windows payloads", () => {
    ...
    return Effect.scoped(
      Effect.gen(function* () {
        const fixture = yield* makeWindowsPayloadFixture({ copyUnpackedNatives: true });
        // Fixture packing has its own native probe; count only validation calls.
        commands.length = 0;
        assert.equal(yield* HostProcessArchitecture, "x64");
        yield* validateWindowsPackagedPayload({
          stageDistDir: fixture.stageDistDir,
          appExecutableName: fixture.appExecutableName,
          targetArch: "arm64",
        ...
        assert.isFalse(
          commands.some(
            (command) =>
              command.command ===
                NodePath.join(fixture.packagedAppDir, fixture.appExecutableName) &&
