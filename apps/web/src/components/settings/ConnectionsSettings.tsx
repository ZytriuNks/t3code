...
import { type I18nContextValue, useConnectionStatusCopy, useI18n } from "../../i18n/I18nProvider";
...
const PAIRING_SCOPE_OPTIONS: ReadonlyArray<{
...
}> = [
  {
    ...
    titleKey: "settings.connections.viewEnvironment",
    descriptionKey: "settings.connections.viewEnvironmentDescription",
  },
  {
    scope: AuthOrchestrationOperateScope,
    titleKey: "settings.connections.operateTasks",
    descriptionKey: "settings.connections.operateTasksDescription",
  },
  {
    scope: AuthTerminalOperateScope,
    titleKey: "settings.connections.useTerminals",
    descriptionKey: "settings.connections.useTerminalsDescription",
  },
  {
    scope: AuthReviewWriteScope,
    titleKey: "settings.connections.writeReviews",
    descriptionKey: "settings.connections.writeReviewsDescription",
  },
  {
    scope: AuthAccessReadScope,
    titleKey: "settings.connections.viewAccess",
    descriptionKey: "settings.connections.viewAccessDescription",
  },
  {
    scope: AuthAccessWriteScope,
    titleKey: "settings.connections.manageAccess",
    descriptionKey: "settings.connections.manageAccessDescription",
  },
  {
    scope: AuthRelayReadScope,
    titleKey: "settings.connections.viewRelay",
    descriptionKey: "settings.connections.viewRelayDescription",
  },
  {
    scope: AuthRelayWriteScope,
    titleKey: "settings.connections.manageRelay",
    descriptionKey: "settings.connections.manageRelayDescription",
  },
];
...
function AccessScopeSummary({
  ...
  const { t } = useI18n();
  const scopeCountLabel = `${scopes.length} ${t(
    scopes.length === 1 ? "settings.connections.scope" : "settings.connections.scopes",
  )}`;
    ...
    <Popover>
      <PopoverTrigger
        ...
        render={
          <button
            type="button"
            aria-label={t("settings.connections.showScopes", {
              label,
...
function parseManualDesktopSshTarget(
  ...
  if (rawHost.length === 0) {
    throw new Error(t("settings.connections.sshHostRequired"));
  }
  ...
  if (port !== null && (!Number.isInteger(port) || port <= 0 || port > 65_535)) {
    throw new Error(t("settings.connections.sshPortRange"));
  }
...
function parseRemotePairingFields(
  ...
  if (!host) {
    throw new Error(t("settings.connections.enterBackendHost"));
  }
  if (!pairingCode) {
    throw new Error(t("settings.connections.enterPairingCode"));
  }
  return { host, pairingCode };
}

function formatDesktopSshConnectionError(error: unknown, t: I18nContextValue["t"]): string {
  const fallback = t("settings.connections.sshConnectionFailed");
...
function summarizeAuthorizedClients(
  ...
  const parts = [
    `${sessions.length} ${t(
      sessions.length === 1 ? "settings.connections.client" : "settings.connections.clients",
    )}`,
    links.length > 0
      ? `${links.length} ${t(
          links.length === 1
            ? "settings.connections.pairingLinkCount"
            : "settings.connections.pairingLinksCount",
        )}`
      : null,
  ];
...
function endpointShareHint(
  ...
  if (isHostedAppPairingUrl(url)) {
    return t("settings.connections.noInstallNeeded");
  }
  switch (endpoint.reachability) {
    case "lan":
      return t("settings.connections.devicesSameNetwork");
    case "private-network":
      return t("settings.connections.devicesPrivateNetwork");
    case "public":
      return t("settings.connections.reachableAnywhere");
    case "loopback":
      return t("settings.connections.clientsThisMachine");
  }
}
...
const PairingLinkListRow = memo(function PairingLinkListRow({
  ...
  const { copyToClipboard } = useCopyToClipboard<{
    ...
    onCopy: ({ kind }) => {
      toastManager.add({
        ...
        title:
          kind === "hosted-link"
            ? t("settings.connections.hostedAppLinkCopied")
            : kind === "link"
              ? t("settings.connections.pairingUrlCopied")
              : t("settings.connections.pairingCodeCopied"),
        description:
          kind === "hosted-link"
            ? t("settings.connections.openOnDevice")
            : kind === "link"
              ? t("settings.connections.openInClient")
              : t("settings.connections.pasteIntoClient"),
      });
    },
      ...
      toastManager.add(
        stackedThreadToast({
          ...
          title: canCopyToClipboard
            ? kind === "hosted-link"
              ? t("settings.connections.couldNotCopyHostedAppLink")
              : kind === "link"
                ? t("settings.connections.couldNotCopyPairingUrl")
                : t("settings.connections.couldNotCopyPairingCode")
            : t("settings.connections.clipboardUnavailable"),
          description: canCopyToClipboard
            ? error.message
            : t("settings.connections.showingFullValue"),
        }),
      );
    },
  });
  ...
  const primaryLabel = pairingLink.label ?? t("settings.connections.pairingLink");
        ...
        <div className="min-w-0 flex-1 space-y-1">
          ...
          <p className="text-xs text-muted-foreground">
            ...
            <AccessScopeSummary
              ...
              label={t("settings.connections.pairingLink")}
            />
          </p>
          {!credential ? (
            ...
            {!credential ? null : canCopyToClipboard ? (
              shareablePairingUrl ? null : (
                <Button size="xs" variant="outline" onClick={handleCopyCode}>
                  {t("settings.connections.copyCode")}
                </Button>
              )
            ) : (
              <DialogTrigger render={<Button size="xs" variant="outline" />}>
                {shareablePairingUrl
                  ? t("settings.connections.showLink")
                  : t("settings.connections.showCode")}
              </DialogTrigger>
            )}
                ...
                <DialogTitle>
                  {isRevealValueUrl
                    ? isRevealValueHostedAppPairingUrl
                      ? t("settings.connections.hostedAppPairingLink")
                      : t("settings.connections.pairingLink")
                    : t("settings.connections.pairingCode")}
                </DialogTitle>
                <DialogDescription>
                  {isRevealValueUrl
                    ? isRevealValueHostedAppPairingUrl
                      ? t("settings.connections.clipboardHostedHint")
                      : t("settings.connections.clipboardUrlHint")
                    : t("settings.connections.clipboardCodeHint")}
                </DialogDescription>
              </DialogHeader>
                ...
                {isRevealValueUrl && isRevealValueQrShareable ? (
                  <div className="flex justify-center rounded-xl border border-border/60 bg-muted/30 p-4">
                    <QRCodeSvg
                      ...
                      level="M"
                      marginSize={2}
                      title={t("settings.connections.pairingQrTitle")}
                    />
                  </div>
                ) : null}
              </DialogPanel>
        ...
        >
          <div className="min-w-0 flex-1 space-y-3">
            ...
            <Button size="xs" variant="ghost" onClick={handleCopyCode}>
              {t("settings.connections.copyCodeOnly")}
            </Button>
          </div>
        ...
        <div className="min-w-0 flex-1 space-y-1">
          ...
          <p className="text-xs text-muted-foreground">
            ...
            <AccessScopeSummary
              ...
              label={t("settings.connections.clientScopes")}
            />
          </p>
        </div>
        <div className="flex w-full shrink-0 items-center gap-2 sm:w-auto sm:justify-end">
...
const AuthorizedClientsHeaderAction = memo(function AuthorizedClientsHeaderAction({
  ...
  const handleCreatePairingLink = useCallback(async () => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : t("settings.connections.failedCreatePairingUrl");
      toastManager.add(
        stackedThreadToast({
          type: "error",
          title: t("settings.connections.couldNotCreatePairingUrl"),
          description: message,
        }),
      );
    } finally {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      >
        {isRevokingOtherClients
          ? t("settings.connections.revoking")
          : t("settings.connections.revokeOthers")}
      </Button>
      <Dialog
        open={dialogOpen}
        onOpenChange={(open) => {
        ...
        <DialogTrigger
          render={
            <Button size="xs" variant="default">
              ...
              {t("settings.connections.createLink")}
            </Button>
          }
        />
        <DialogPopup className="max-w-md">
          <DialogHeader>
            <DialogTitle>{t("settings.connections.createPairingLink")}</DialogTitle>
            <DialogDescription>
              {t("settings.connections.createPairingLinkDescription")}
            </DialogDescription>
          </DialogHeader>
          <DialogPanel className="space-y-5">
            <label className="block">
              <span className="mb-1.5 block text-xs font-medium text-foreground">
                {t("settings.connections.clientLabelOptional")}
              </span>
              <Input
                value={pairingLabel}
                onChange={(event) => setPairingLabel(event.target.value)}
                placeholder={t("settings.connections.clientLabelPlaceholder")}
            ...
            <section className="space-y-3">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t("settings.connections.limitPairedClient")}
                  </p>
                </div>
                <div className="flex gap-1">
                  ...
                  >
                    {t("settings.connections.readOnly")}
                  </Button>
                  <Button
                    size="xs"
                    variant="outline"
              ...
              {pairingScopes.length === 0 ? (
                <p className="text-xs text-destructive">
                  {t("settings.connections.selectPermission")}
                </p>
              ) : pairingScopes.includes(AuthAccessWriteScope) ? (
                <p className="text-xs text-warning">{t("settings.connections.accessWarning")}</p>
              ) : null}
            </section>
          </DialogPanel>
          <DialogFooter variant="bare">
            ...
            >
              {isCreatingPairingLink
                ? t("settings.connections.creating")
                : t("settings.connections.createLink")}
            </Button>
          </DialogFooter>
        </DialogPopup>
      ...
      {pairingLinks.length === 0 && clientSessions.length === 0 && !isLoading ? (
        <div className={accessRowClassName(presentation)}>
          <p className="text-xs text-muted-foreground/60">
            {t("settings.connections.noPairingClients")}
          </p>
        </div>
      ) : null}
    </>
...
function NetworkAccessDescription({
    ...
    <>
      ...
      {hiddenEndpointCount > 0 ? (
        <span className="shrink-0 text-xs font-medium">
          {expanded ? t("settings.connections.hideEndpoints") : `+${hiddenEndpointCount}`}
        </span>
      ) : null}
    </>
  );
  ...
  return (
    <span className="inline-flex min-w-0 max-w-full items-baseline gap-1">
      <span className="shrink-0">{t("settings.connections.reachableAtLabel")}</span>
      {hiddenEndpointCount > 0 ? (
        <button
          type="button"
...
function savedBackendStatus(
  ...
  if (!environment.entry.enabled && environment.connection.phase !== "unsupported")
    return { text: t("settings.connections.off"), tone: "muted" };
    ...
    case "reconnecting":
      return {
        text: connection.error
          ? `${t("settings.connections.reconnecting")}: ${connection.error}`
          : t("settings.connections.reconnecting"),
        tone: "error",
      };
    case "unsupported":
      return { text: t("settings.connections.clientNotSupported"), tone: "error" };
    case "error":
      return {
        text: connection.error
          ? `${t("settings.connections.connectionFailed")}: ${connection.error}`
...
function SavedBackendListRow({
  ...
  const { copyToClipboard: copyTraceIdToClipboard } = useCopyToClipboard<{ traceId: string }>({
    ...
    onCopy: ({ traceId }) => {
      toastManager.add({
        type: "success",
        title: t("settings.connections.traceIdCopied"),
        description: traceId,
      });
    },
    onError: (error) => {
      toastManager.add(
        stackedThreadToast({
          type: "error",
          title: t("settings.connections.couldNotCopyTraceId"),
  ...
  const subtitleText = [
    ...
    resumingServerUpdate ? t("settings.connections.restarting") : status.text,
    ...
    >
      {showUpdateAction ? (
        <ServerUpdateAction
          ...
          serverLabel={`${environment.label} ${t("settings.connections.server")}`}
        ...
        <TooltipTrigger
          render={
            <Switch
              ...
              aria-label={`${enabled ? t("settings.connections.switchOff") : t("settings.connections.switchOn")} ${environment.label}`}
        ...
        <MenuTrigger
          render={
            <Button
              ...
              disabled={isRemoving}
              aria-label={t("settings.connections.moreActionsFor", {
        ...
        <MenuPopup align="end" className="min-w-52">
          ...
          <MenuItem variant="destructive" onClick={() => onRemove(environment)}>
            {isRemoving
              ...
              : t("settings.connections.removeFromDevice")}
          </MenuItem>
        </MenuPopup>
      </Menu>
    </EnvironmentRow>
...
function CloudLinkSwitch({
  ...
  const control = (
    <Switch
      aria-label={ariaLabel ?? t("settings.connections.enableT3Connect")}
      checked={checked}
      disabled={disabled}
...
function ConfiguredCloudLinkRow({ canManageRelay }: { readonly canManageRelay: boolean }) {
  ...
  const disabledReason = !isSignedIn
    ? t("settings.connections.signInToManageT3Connect")
    : !canManageRelay
      ? t("settings.connections.noPermissionT3Connect")
      : null;
  ...
  const updateManagedTunnel = async (enabled: boolean) => {
    ...
    if (ok) {
      ...
      toastManager.add({
        type: "success",
        title: enabled
          ? t("settings.connections.t3ConnectLinked")
          : publishAgentActivity
            ? t("settings.connections.t3ConnectTunnelDisabled")
            : t("settings.connections.t3ConnectUnlinked"),
        description: enabled
          ? t("settings.connections.t3ConnectAvailable")
          : publishAgentActivity
            ? t("settings.connections.t3ConnectTunnelRemoved")
            : t("settings.connections.t3ConnectUnavailable"),
      });
    }
    setIsUpdating(false);
  };
  ...
  const updatePublishAgentActivity = async (enabled: boolean) => {
    ...
    if (ok) {
      toastManager.add({
        type: "success",
        title: enabled
          ? t("settings.connections.agentActivityEnabled")
          : t("settings.connections.agentActivityDisabled"),
        description: enabled
          ? t("settings.connections.agentActivityEnabledDescription")
          : t("settings.connections.agentActivityDisabledDescription"),
      });
    }
    ...
    <>
      {window.desktopBridge ? (
        <SettingsRow
          ...
          description={
            managedTunnelActive
              ? t("settings.connections.t3ConnectAvailableOtherDevices")
              : t("settings.connections.t3ConnectMakeAvailable")
          }
      ...
      <SettingsRow
        ...
        description={t("settings.connections.publishAgentActivityDescription")}
        control={
          <CloudLinkSwitch
            ariaLabel={t("settings.connections.agentActivityToggle")}
...
function EmptyRemoteEnvironments({ cloudEnabled = true }: { readonly cloudEnabled?: boolean }) {
  ...
  return (
    <Empty className="min-h-52">
      ...
      <EmptyHeader>
        <EmptyTitle>{t("settings.connections.noSavedRemoteEnvironments")}</EmptyTitle>
        <EmptyDescription>
          {cloudEnabled
            ? t("settings.connections.addEnvironmentOrT3Connect")
            : t("settings.connections.addEnvironmentOnly")}
        </EmptyDescription>
      </EmptyHeader>
    </Empty>
  );
...
export function ConnectionsSettings() {
  ...
  const handleDesktopServerExposureChange = useCallback(
    async (checked: boolean) => {
      ...
      try {
        ...
        const message =
          error instanceof Error ? error.message : t("settings.connections.failedNetworkAccess");
        setIsDesktopServerExposureDialogOpen(false);
        setDesktopServerExposureMutationError(message);
        toastManager.add(
          stackedThreadToast({
            type: "error",
            title: t("settings.connections.couldNotUpdateNetworkAccess"),
            description: message,
          }),
        );
        setIsUpdatingDesktopServerExposure(false);
  ...
  const handleConfirmTailscaleServeSetup = useCallback(async () => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : t("settings.connections.failedTailscaleSetup");
      setDesktopServerExposureMutationError(message);
      toastManager.add(
        stackedThreadToast({
          type: "error",
          title: t("settings.connections.couldNotSetupTailscale"),
          description: message,
        }),
      );
    } finally {
  ...
  const handleConfirmTailscaleServeDisable = useCallback(async () => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : t("settings.connections.failedTailscaleDisable");
      setDesktopServerExposureMutationError(message);
      toastManager.add(
        stackedThreadToast({
          type: "error",
          title: t("settings.connections.couldNotDisableTailscale"),
          description: message,
        }),
      );
    } finally {
      ...
      try {
        ...
        const message =
          error instanceof Error
            ? error.message
            : t("settings.connections.failedRevokePairingLink");
        setDesktopAccessManagementMutationError(message);
        toastManager.add(
          stackedThreadToast({
            type: "error",
            title: t("settings.connections.couldNotRevokePairingLink"),
            description: message,
          }),
        );
      } finally {
    ...
    async (sessionId: ServerClientSessionRecord["sessionId"]) => {
      ...
      try {
        ...
        const message =
          error instanceof Error
            ? error.message
            : t("settings.connections.failedRevokeClientAccess");
        setDesktopAccessManagementMutationError(message);
        toastManager.add(
          stackedThreadToast({
            type: "error",
            title: t("settings.connections.couldNotRevokeClientAccess"),
            description: message,
          }),
        );
      } finally {
  ...
  const handleRevokeOtherDesktopClients = useCallback(async () => {
    ...
    try {
      ...
      toastManager.add({
        ...
        title:
          revokedCount === 1
            ? t("settings.connections.revokedOtherClient")
            : t("settings.connections.revokedClients", { count: revokedCount }),
        description: t("settings.connections.otherClientsNeedNewLink"),
      });
    } catch (error) {
      const message =
        error instanceof Error ? error.message : t("settings.connections.failedRevokeOtherClients");
      setDesktopAccessManagementMutationError(message);
      toastManager.add(
        stackedThreadToast({
          type: "error",
          title: t("settings.connections.couldNotRevokeOtherClients"),
          description: message,
        }),
      );
    } finally {
  ...
  const connectSavedBackendSshTarget = useCallback(
    async (target: DesktopSshEnvironmentTarget) => {
      ...
      toastManager.add({
        type: "success",
        title: t("settings.connections.environmentConnected"),
        description: t("settings.connections.sshTunnelReady", { alias: target.alias }),
      });
      setIsAddingSavedBackend(false);
    },
  ...
  const handleAddSavedBackend = useCallback(async () => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : t("settings.connections.failedAddBackend");
      setSavedBackendError(message);
      toastManager.add(
        stackedThreadToast({
          type: "error",
          title: t("settings.connections.couldNotAddBackend"),
          description: message,
        }),
      );
      setIsAddingSavedBackend(false);
    ...
    toastManager.add({
      type: "success",
      title: t("settings.connections.backendAdded"),
      description: t("settings.connections.backendReconnectOnStartup"),
    });
    setIsAddingSavedBackend(false);
  }, [
    connectPairing,
    ...
    async (environmentId: EnvironmentId, enabled: boolean) => {
      ...
      if (result._tag === "Failure" && !isAtomCommandInterrupted(result)) {
        ...
        const message =
          error instanceof Error
            ? error.message
            : t("settings.connections.backendSwitchError", {
                state: enabled
                  ? t("settings.connections.switchOn")
                  : t("settings.connections.switchOff"),
              });
        ...
        toastManager.add(
          stackedThreadToast({
            type: "error",
            title: t("settings.connections.couldNotSwitchBackend", {
              state: enabled
                ? t("settings.connections.switchOn")
                : t("settings.connections.switchOff"),
            }),
  ...
  const handleRemoveSavedBackend = useCallback(
    async (environment: EnvironmentPresentation) => {
      ...
      if (result._tag === "Failure" && !isAtomCommandInterrupted(result)) {
        const error = squashAtomCommandFailure(result);
        const message =
          error instanceof Error ? error.message : t("settings.connections.failedRemoveBackend");
        setSavedBackendError(message);
        toastManager.add(
          stackedThreadToast({
            type: "error",
            title: t("settings.connections.couldNotRemoveBackend"),
            description: message,
          }),
        );
      }
    ...
    <div className="space-y-4">
      ...
      >
        ...
        {isAddingSavedBackend
          ...
          : t("settings.connections.addEnvironment")}
      </Button>
    </div>
  );
    ...
    <div className="space-y-4">
      <div className="space-y-3">
        <div className="block">
          ...
          >
            {t("settings.connections.sshHostAlias")}
          </label>
          <Autocomplete
          ...
          >
            <AutocompleteInput
              ...
              placeholder={t("settings.connections.searchHosts")}
              disabled={isAddingSavedBackend}
              spellCheck={false}
            />
            {hasSshHostSuggestionContent ? (
              <AutocompletePopup>
                {isLoadingDiscoveredSshHosts ? (
                  <div className="px-3 py-2 text-xs text-muted-foreground">
                    {t("settings.connections.loadingHosts")}
                  </div>
                  ...
                  <AutocompleteEmpty className="break-all px-3 py-2 text-xs">
                    {t("settings.connections.noHostsMatch", {
                      host: savedBackendSshHost.trim(),
                    })}
                  </AutocompleteEmpty>
                )}
              </AutocompletePopup>
  ...
  const renderNetworkAccessToggle = () => (
    <Switch
      ...
      aria-label={t("settings.connections.enableNetworkAccess")}
    />
  );
  ...
  const applyWslSettingChange = useCallback(
    async (apply: () => Promise<DesktopWslState>) => {
      ...
      try {
        ...
        const message =
          error instanceof Error ? error.message : t("settings.connections.failedWslUpdate");
        setDesktopWslMutationError(message);
        toastManager.add(
          stackedThreadToast({
            type: "error",
            title: t("settings.connections.couldNotChangeWsl"),
            description: message,
          }),
        );
        refreshDesktopWslState();
  ...
  const renderWslRow = () => {
      ...
      if (
        ...
        return (
          <SettingsRow
            ...
            description={t("settings.connections.wslLoadFailed")}
            ...
            control={
              ...
              >
                {isLoadingWslState
                  ? t("settings.connections.retrying")
    ...
    if (!desktopWslState.available) {
      return (
        <SettingsRow
          {...searchableSetting("wsl-backend", t)}
          description={t("settings.connections.wslUnavailableWindows")}
          status={
            desktopWslError ? (
          ...
          control={
            ...
            >
              {t("settings.connections.switchToWindows")}
            </Button>
          }
        />
      );
    ...
    const selectLabel =
      selectValue === BACKEND_VALUE_WSL_OFF
        ...
        : selectValue === BACKEND_VALUE_DEFAULT_WSL
          ? t("settings.connections.defaultDistro")
          : selectValue;
    return (
      <>
        <SettingsRow
          {...searchableSetting("wsl-backend", t)}
          description={t("settings.connections.wslDescription")}
          status={
            desktopWslError ? (
              <span className="block text-destructive">{desktopWslError}</span>
            ) : desktopWslState.preflightError ? (
              <span className="block text-destructive">
                {t("settings.connections.wslPreflightFailed", {
                ...
                {desktopWslState.distros.length === 0 ? (
                  ...
                  desktopWslState.distros.map((distro) => (
                    <SelectItem hideIndicator key={distro.name} value={distro.name}>
                      {distro.name}
                      {distro.isDefault ? ` (${t("settings.connections.default")})` : ""}
                    </SelectItem>
                  ))
                )}
              </SelectPopup>
        ...
        {desktopWslState.enabled ? (
          <SettingsRow
            title={t("settings.connections.wslOnly")}
            description={t("settings.connections.wslOnlyDescription")}
            ...
            control={
              <Switch
                ...
                aria-label={t("settings.connections.runWslOnly")}
              />
            }
          />
        ) : null}
      ...
      description={
        tailscaleHttpsEndpoint
          ? tailscaleHttpsEndpoint.status === "available"
            ...
            : t("settings.connections.tailscaleHttpsDescription")
          : t("settings.connections.runTailscaleToSetup")
      }
      control={
        tailscaleHttpsEndpoint ? (
          <Switch
            ...
            aria-label={t("settings.connections.enableTailscaleHttps")}
          />
        ) : null
      }
    />
          ...
          <NetworkAccessDescription
            ...
            fallback={
              desktopServerExposureState?.endpointUrl
                ? t("settings.connections.reachableAt", {
                    endpoint: desktopServerExposureState.endpointUrl,
                  })
                : desktopServerExposureState?.advertisedHost
                  ? t("settings.connections.exposedAllInterfacesPairing", {
                      host: desktopServerExposureState.advertisedHost,
                    })
                  : t("settings.connections.exposedAllInterfaces")
            }
          />
        ) : desktopServerExposureState ? (
          t("settings.connections.limitedThisMachine")
        ) : (
  ...
  const renderDisabledNetworkAccessRow = () => (
    <SettingsRow
      ...
      description={
        currentAuthPolicy === "remote-reachable"
          ? t("settings.connections.remoteAlreadyConfigured")
          : t("settings.connections.onlyThisMachine")
      }
      control={
        <Tooltip>
          <TooltipTrigger
          ...
          />
          <TooltipPopup side="top">
            {t("settings.connections.networkExposureRestartHint")}
          </TooltipPopup>
        </Tooltip>
      }
    />
    ...
    <>
      {desktopBridge || canManageLocalBackend ? (
        <>
                  ...
                  <MenuTrigger
                    render={
                      <Button
                        ...
                        size="icon-xs"
                        className="text-muted-foreground hover:text-foreground"
                        aria-label={t("settings.connections.primaryMachineActions")}
                      />
                    }
                  >
                    <EllipsisIcon className="size-3.5" />
                ...
                control={
                  ...
                  primaryServerUpdateState.status !== "running" ? (
                    <ServerUpdateAction
                      ...
                      serverLabel={
                        primaryEnvironment
                          ? `${primaryEnvironment.label} ${t("settings.connections.server")}`
                          : t("settings.connections.server")
                      }
                      ...
                      label={
                        primaryServerUpdateState.status === "failed"
                          ? t("settings.connections.retryUpdate")
                          : t("settings.connections.versionUpdate", {
          ...
          {isLocalBackendRemotelyReachable ? (
            <FoldedSettingsSection
              ...
              title={t("settings.connections.authorizedClients")}
              ...
              <AlertDialogHeader>
                <AlertDialogTitle>
                  {pendingDesktopServerExposureMode === "network-accessible"
                    ? t("settings.connections.enableNetworkAccessQuestion")
                    : t("settings.connections.disableNetworkAccessQuestion")}
                </AlertDialogTitle>
                <AlertDialogDescription>
                  {pendingDesktopServerExposureMode === "network-accessible"
                    ? t("settings.connections.restartExposeNetwork")
                    : t("settings.connections.restartLimitMachine")}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
                ...
                >
                  {isUpdatingDesktopServerExposure ? (
                    <>
                      ...
                      {t("settings.connections.restartingAction")}
                    </>
                  ) : pendingDesktopServerExposureMode === "network-accessible" ? (
                ...
                <AlertDialogTitle>
                  {pendingWslChange?.kind === "disable"
                    ? pendingWslChange.wasWslOnly
                      ? t("settings.connections.turnOffWslWindowsQuestion")
                      : t("settings.connections.disableWslQuestion")
                    : pendingWslChange?.kind === "distro"
                      ? t("settings.connections.switchWslDistroQuestion")
                      : pendingWslChange?.kind === "enable"
                        ? t("settings.connections.startWslBackend")
                        : pendingWslChange?.nextValue
                          ? t("settings.connections.runOnlyWslQuestion")
                          : t("settings.connections.reenableWindowsQuestion")}
                </AlertDialogTitle>
                <AlertDialogDescription>
                  {pendingWslChange?.kind === "disable"
                    ? pendingWslChange.wasWslOnly
                      ? t("settings.connections.wslRestartWindows")
                      : t("settings.connections.wslStopDescription")
                    : pendingWslChange?.kind === "distro"
                      ? t("settings.connections.wslSwitchDistroDescription")
                      : pendingWslChange?.kind === "enable"
                        ? t("settings.connections.wslChooseModeDescription")
                        : pendingWslChange?.nextValue
                          ? t("settings.connections.wslOnlyRestartDescription")
                          : t("settings.connections.wslBothRestartDescription")}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
                  ...
                  <>
                    ...
                    >
                      {isUpdatingWslBackend ? (
                        <>
                          <Spinner className="size-3.5" />
                          {t("settings.connections.applying")}
                        </>
                      ) : (
                        t("settings.connections.useOnlyWsl")
                      )}
                    </Button>
                    <Button
                      variant="default"
                    ...
                    >
                      {isUpdatingWslBackend ? (
                        ...
                        t("settings.connections.runBothBackends")
                      )}
                    </Button>
                  </>
                ) : (
                  ...
                  >
                    {isUpdatingWslBackend ? (
                      ...
                      pendingWslChange.wasWslOnly ? (
                        ...
                        t("settings.connections.disableWsl")
                      )
                    ) : pendingWslChange?.kind === "distro" ? (
                      t("settings.connections.switchDistro")
                    ) : pendingWslChange?.nextValue ? (
                      t("settings.connections.restartAndEnable")
                    ) : (
                      t("settings.connections.restartAndDisable")
          ...
          >
            <AlertDialogPopup>
              <AlertDialogHeader>
                <AlertDialogTitle>
                  {t("settings.connections.disableTailscaleHttpsQuestion")}
                </AlertDialogTitle>
                <AlertDialogDescription>
                  {t("settings.connections.tailscaleDisableDescription")}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
          ...
          >
            <DialogPopup className="max-w-md">
              <DialogHeader>
                <DialogTitle>{t("settings.connections.setUpTailscaleHttpsQuestion")}</DialogTitle>
                <DialogDescription>
                  {t("settings.connections.tailscaleSetupDescription")}
                </DialogDescription>
              </DialogHeader>
              <DialogPanel className="space-y-4">
                <label className="block">
                  <span className="text-sm font-medium text-foreground">
                    {t("settings.connections.httpsPort")}
                  </span>
                  <Input
                    className="mt-2"
                    type="number"
                ...
                {!isTailscaleServePortValid ? (
                  <p className="mt-2 text-xs text-destructive">
                    {t("settings.connections.enterValidPort")}
                  </p>
                ) : null}
                <div className="rounded-md border border-border/70 bg-muted/20 px-3 py-2">
                  <p className="text-xs font-medium text-muted-foreground">
                    {t("settings.connections.httpsEndpoint")}
                  </p>
                  <Tooltip>
                    <TooltipTrigger
                      render={
                        <p className="mt-1 truncate text-sm text-foreground">
                          {pendingTailscaleServeBaseUrl ??
                            t("settings.connections.pendingMagicDnsEndpoint")}
                        </p>
                      }
                    />
                    {pendingTailscaleServeBaseUrl ? (
              ...
              <DialogFooter>
                ...
                >
                  {isUpdatingTailscaleServe ? (
                    ...
                    t("settings.connections.enableTailscaleHttps")
                  )}
                </Button>
              </DialogFooter>
            </DialogPopup>
        ...
        <SettingsSection {...searchableSetting("connections-environment", t)}>
          <SettingsRow
            title={t("settings.connections.administrativeAccess")}
            description={t("settings.connections.administrativeAccessDescription")}
          />
          <CloudLinkRow canManageRelay={canManageRelay} />
        </SettingsSection>
      )}
                    ...
                    <DialogTrigger
                      render={
                        <Button
                          ...
                          variant="ghost"
                          className="font-normal text-muted-foreground/60 hover:text-muted-foreground"
                          aria-label={t("settings.connections.addEnvironment")}
                        >
                          <PlusIcon className="size-3" />
                          <span>{t("settings.connections.addEnvironment")}</span>
                        </Button>
                      }
                    />
                  }
                />
                <TooltipPopup side="top">{t("settings.connections.addEnvironment")}</TooltipPopup>
              </Tooltip>
              <DialogPopup className="max-h-[80dvh] sm:max-w-3xl">
                <DialogHeader>
                  <DialogTitle>{t("settings.connections.addEnvironmentTitle")}</DialogTitle>
                  <DialogDescription>
                    {t("settings.connections.pairAnotherEnvironment")}
                  </DialogDescription>
                </DialogHeader>
                <DialogPanel>
                  <div className="space-y-4">
                    <div className="grid gap-3 sm:grid-cols-2">
                      {renderConnectionModeCard({
                        mode: "remote",
                        title: t("settings.connections.remoteLink"),
                        description: t("settings.connections.remoteLinkDescription"),
                        icon: <ChevronsLeftRightEllipsisIcon aria-hidden className="size-4" />,
                      })}
                      {desktopBridge
                        ? renderConnectionModeCard({
                            mode: "ssh",
                            title: t("settings.connections.ssh"),
                            description: t("settings.connections.sshDescription"),
