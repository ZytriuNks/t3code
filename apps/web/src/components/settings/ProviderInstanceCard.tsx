...
function ProviderAuthEmail(props: { readonly email: string | undefined }) {
  ...
  return (
    <RedactedSensitiveText
      value={email}
      ariaLabel={t("settings.providers.variable.toggleEmail")}
      revealTooltip={t("settings.providers.variable.revealEmail")}
      hideTooltip={t("settings.providers.variable.hideEmail")}
...
function ProviderEnvironmentFieldRow(props: {
  ...
  const placeholder = props.variable?.valueRedacted
    ? t("settings.providers.variable.storedSecretHint")
        ...
        <div className="flex w-full min-w-0 items-center gap-2 @min-[32rem]/settings-row:w-56">
          ...
          {props.variable ? (
            <Button
              ...
              aria-label={t("settings.providers.variable.clearField", { label: props.field.label })}
            >
    ...
    >
      {rows.length > 0 ? (
        <div className="mt-3 min-w-0 space-y-2 pb-2">
          {rows.map((variable, index) => (
            <div key={variable.id} className="flex min-w-0 flex-wrap items-center gap-1.5">
              <DraftInput
                ...
                spellCheck={false}
                aria-label={t("settings.providers.variable.nameAria", { index: index + 1 })}
              />
              ...
              <DraftInput
                ...
                placeholder={
                  variable.valueRedacted
                    ? t("settings.providers.variable.storedSecret")
                    : t("settings.providers.variable.value")
                }
                spellCheck={false}
                aria-label={t("settings.providers.variable.valueAria", { index: index + 1 })}
              />
              <Tooltip>
                <TooltipTrigger
                  render={
                    <Button
                      ...
                      aria-label={t("settings.providers.variable.markSensitive", {
                ...
                <TooltipPopup side="top">
                  {t(
                    variable.sensitive
                      ? "settings.providers.variable.sensitive"
                      : "settings.providers.variable.plainText",
                  )}
                </TooltipPopup>
              </Tooltip>
              <Button
                ...
                aria-label={t("settings.providers.variable.remove", {
                  name: variable.name || index + 1,
                })}
              >
          ...
          <p className="text-xs text-muted-foreground">{t("settings.providers.variable.notice")}</p>
        </div>
      ) : null}
    </SettingsRow>
  );
...
export function ProviderInstanceCard({
  ...
  const { copyToClipboard } = useCopyToClipboard<{ providerName: string }>({
    onCopy: ({ providerName }) => {
      toastManager.add({
        type: "success",
        title: t("settings.providers.card.updateCommandCopied", { provider: providerName }),
        description: t("settings.providers.card.runUpdateCommand"),
      });
    },
    onError: (error, { providerName }) => {
      toastManager.add(
        stackedThreadToast({
          type: "error",
          title: t("settings.providers.card.copyUpdateFailed", { provider: providerName }),
  ...
  const inlineStatusDetail = hasCompatibilityWarning
    ? compatibility?.status === "broken"
      ? t("settings.providers.summary.incompatible")
      : compatibility?.status === "unsupported"
        ? t("settings.providers.summary.unsupported")
        : t("settings.providers.summary.limitedSupport")
    : summary.detail;
      ...
      >
        ...
        >
          <button
            ...
            onClick={onSelect}
            aria-label={t("settings.providers.card.select", { provider: displayName })}
              ...
              {versionAdvisory ? (
                hasCompatibilityWarning ? (
                  ...
                  <Tooltip>
                    <TooltipTrigger
                      render={
                        <Button
                          ...
                          aria-label={t("settings.providers.card.copyNamedUpdateCommand", {
                    ...
                    />
                    <TooltipPopup side="top">
                      {t("settings.providers.card.copyUpdateCommand")}
                    </TooltipPopup>
                  </Tooltip>
                ) : (
                  <span
                    role="img"
                    aria-label={t("settings.providers.card.updateAvailable")}
        ...
        <span className="flex h-5 shrink-0 items-center">
          <Switch
            ...
            aria-label={t("settings.providers.card.enable", { provider: displayName })}
          />
        </span>
      </div>
    );
      ...
      >
                  ...
                  <PopoverTrigger
                    render={
                      <Button
                        ...
                        size="icon-xs"
                        variant="ghost-muted"
                        aria-label={t("settings.providers.version.viewDetails", {
              ...
              <div className="grid min-w-0 gap-3">
                  ...
                  >
                    ...
                    {isUpdating
                      ...
                      : versionAdvisory.targetVersion
                        ? t("settings.providers.card.installVersion", {
                ...
                {onRunVersionAction && updateCommand ? (
                  <div className="flex items-center gap-2 text-[10px] font-medium uppercase tracking-wider text-muted-foreground">
                    ...
                    {t("settings.providers.card.updateManually")}
                    <span aria-hidden className="h-px flex-1 bg-border" />
                  </div>
                ) : null}
                {updateCommand ? (
                  <div className="flex min-w-0 items-center gap-1 rounded-md border border-border/70 bg-muted/40 py-0.5 pr-0.5 pl-2">
                    ...
                    <Tooltip>
                      <TooltipTrigger
                        render={
                          <Button
                            ...
                            aria-label={t("settings.providers.card.copyUpdateCommand")}
                          >
                      ...
                      />
                      <TooltipPopup side="top">
                        {t("settings.providers.card.copyCommand")}
                      </TooltipPopup>
                    </Tooltip>
                  </div>
                ) : null}
        ...
        {onDelete ? (
          <Button
            ...
            disabled={readOnly}
            onClick={onDelete}
            aria-label={t("settings.providers.card.deleteInstance", { id: instanceId })}
          >
            <Trash2Icon />
          </Button>
        ) : null}
  ...
  return (
    <>
            ...
            <>
              ...
              {urlAuthAction && onAcceptUrlAuth ? (
                <div className="grid max-w-xl gap-1.5 pt-1 text-xs">
                  ...
                  >
                    <ExternalLinkIcon />
                    {t("settings.providers.card.continueAuth")}
                  </Button>
                </div>
              ) : null}
            </>
          ...
          control={
            ...
            >
              ...
              <DraftInput
                ...
                placeholder={driverOption?.label ?? t("settings.providers.card.instanceLabel")}
                spellCheck={false}
              />
            </div>
          }
      ...
      {setup || environmentFields.length > 0 ? (
        <SettingsSection title={t("settings.providers.setup")}>
          {setup}
          <div
            inert={readOnly}
            aria-disabled={readOnly || undefined}
      ...
      {!driverOption || deriveProviderSettingsFields(driverOption).length > 0 ? (
        ...
        >
          {driverOption ? (
            ...
            <SettingsRow
              title={t("settings.providers.card.driver")}
              description={
                <span>
                  {t("settings.providers.card.driverUnavailableBefore")}{" "}
                  <code className="text-foreground">{String(instance.driver)}</code>
                  {t("settings.providers.card.driverUnavailableAfter")}
                </span>
              }
            />
          )}
