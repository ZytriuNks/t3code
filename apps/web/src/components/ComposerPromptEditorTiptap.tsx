...
const DEFAULT_COMPOSER_ACCESSIBLE_COPY: ComposerAccessibleCopy = {
...
const RichComposerAccessibleCopyContext = createContext(DEFAULT_COMPOSER_ACCESSIBLE_COPY);
...
function ComposerMentionNodeView({ node }: NodeViewProps) {
  ...
  const accessibleCopy = use(RichComposerAccessibleCopyContext);
  ...
  const chip = (
    <ContextChip
      ...
      aria-label={accessibleCopy.mentionPreview(path)}
  ...
  return (
    <NodeViewWrapper as="span" className={CHIP_NODE_SELECTION_CLASS_NAME}>
      <ContextChipPopover
        ...
        icon={<SkillChipIcon />}
        label={skillLabel}
        accessibleLabel={accessibleCopy.skillLabel(skillLabel)}
        accessibleLabelSuffix={accessibleCopy.accessibleLabelSuffix}
      >
        <div className="space-y-3 p-2 text-sm">
          <p className="font-medium">{skillLabel}</p>
          <p>{skill?.description ?? skillDescription ?? accessibleCopy.skillNoDescription}</p>
          {skill?.path ? (
