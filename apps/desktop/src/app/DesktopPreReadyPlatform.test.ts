  ...
  for (const previousEntry of [undefined, 'Exec="/Applications/deleted-previous.AppImage" %U']) {
    it.effect(
      ...
      () => {
        ...
        let desktopName = "com.t3tools.T3Code.Experimental.desktop";
        ...
        writeFileSyncMock.mockImplementation((path: string, contents: string) => {
          if (path === "/xdg/applications/com.t3tools.T3Code.Experimental.desktop")
        ...
        return Effect.scoped(
          Effect.gen(function* () {
            ...
            assert.equal(identity.desktopName, "com.t3tools.T3Code.Experimental.desktop");
            assert.include(identity.desktopEntry ?? "", 'Exec="/Applications/current.AppImage" %U');
            assert.include(identity.desktopEntry ?? "", "Name=T3 Code (Experimental)");
            assert.include(
              identity.desktopEntry ?? "",
              "MimeType=x-scheme-handler/t3code-experimental;",
            );
          }),
